...
class MCN(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.feature_extractor_method == 'cnn':
            ...
            self.audio_trfm = self.get_transformer_encoder(args.audio_feat_dim, 3)
            self.video_trfm = self.get_transformer_encoder(args.video_feat_dim, 3)
        ...
        if self.recon:
            ...
            self.recon_v = nn.Sequential(
                ...
                nn.Linear(self.recon_size, args.video_feat_dim),
                nn.ReLU(inplace=True)
            )
            self.recon_a = nn.Sequential(
                nn.Linear(args.embd_dim, self.recon_size),
                nn.ReLU(inplace=True),
                nn.Linear(self.recon_size, args.audio_feat_dim),
                nn.ReLU(inplace=True)
            )
        ...
        self.GU_audio = Gated_Embedding_Unit(args.audio_feat_dim, args.embd_dim)
        self.GU_video = Gated_Embedding_Unit(args.video_feat_dim, args.embd_dim)
        self.text_pooling_caption = Sentence_Maxpool(args.text_feat_dim, args.embd_dim)
