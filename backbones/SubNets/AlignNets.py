...
class AlignSubNet(nn.Module):
    def __init__(self, args, mode):
        ...
        if args.method == 'tcl_map':
            args.text_seq_len = args.max_cons_seq_length

        if args.dialogue_mode == 'multi_turn':
            seq_len_t, seq_len_a, seq_len_v = round(args.text_seq_len * (1 + args.context_len)), round(args.audio_seq_len * (1 + args.context_len)), round(args.video_seq_len * (1 + args.context_len))
        else:
    ...
    def __sim(self, text_x, audio_x, video_x):
    ...
    def forward(self, text_x,  audio_x, video_x):
        ...
        if self.mode == 'conv1d':
            ...
            return self.__sim(text_x, audio_x, video_x)
        elif self.mode == 'ctc':
            return self.__ctc(text_x, audio_x, video_x)
