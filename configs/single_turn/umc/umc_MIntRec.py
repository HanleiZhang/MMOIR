    ...
    def _get_hyper_parameters(self, args):
        ...
        if args.multimodal_method == 'umc':
            hyper_parameters = {
                ...
                'pretrain_temperature': [0.2,0.25,0.1],
                'train_temperature_sup': [1.4,1.5,1,2],
                'train_temperature_unsup': [1,0.5,1.5],
                'activation': 'tanh',
                # 'lr_pre': [3e-5,1e-5,2e-5,4e-5],
                'lr_pre': [3e-5],
                'lr': [2e-4,1e-4,4e-4,3e-4],#2e-4不好，往大调
                'delta': [0.05],
                'thres': [0.1],
                'topk': [5],
                'weight_decay': 0.01,
