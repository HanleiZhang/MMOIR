from sklearn.metrics import confusion_matrix, accuracy_score, f1_score, \
    precision_score, recall_score, roc_auc_score, average_precision_score, \
        auc, precision_recall_curve, roc_curve,fowlkes_mallows_score, normalized_mutual_info_score, adjusted_rand_score
...
class OOD_Metrics(object):
    ...
    def __init__(self, args):
        ...
        self.logger = logging.getLogger(args.logger_name)
        self.eval_metrics = ['auroc', 'aupr_in', 'aupr_out', 'fpr95', 'der', 'eer']
...
class CLUSTERING_Metrics(object):
    def __init__(self, args):

        self.logger = logging.getLogger(args.logger_name)
        self.eval_metrics = ['NMI', 'ARI',  'ACC', 'fmi']

    def __call__(self, y_true, y_pred):
        return {
                'NMI': round(normalized_mutual_info_score(y_true, y_pred)*100, 2),
                'ARI': round(adjusted_rand_score(y_true, y_pred)*100, 2),
                'ACC': round(self.clustering_accuracy_score(y_true, y_pred)*100, 2),
    ...
    def clustering_accuracy_score(self, y_true, y_pred):
        ind, w = self.hungray_aligment(y_true, y_pred)
